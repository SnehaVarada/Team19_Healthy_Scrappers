...
public class Diabetes_addon extends BaseTest
{
	...
	public static  String nutrients ;
	 ...
	 public static String foodCategory = "veg";
	 public static String morbidities = "Diabetes";

	@Test
	public static void Diabetes_addon_test() throws InterruptedException, IOException {

		driver.findElement(By.xpath("//input[@id='ctl00_txtsearch']")).sendKeys("Diabetic");
		...
		for (int p = 1; p <= 3; p++) {
			...
			//div[@class='rcc_recipecard']//span[@class='rcc_recipename']/a
			 List<WebElement> RecipeList =
			  driver.findElements(By.xpath("//span[@class='rcc_recipename']/a"));
		   ...
		   driver.findElement(By.xpath("//div[@id='cardholder']//div[3]//a[\" + p + \"]")).click();
...

			{
				...
				boolean isIngrediant = Diabetes_list.checkAddOns(Ingrediants);
				boolean isIngrediantAdds=Diabetes_list.checkAddOns(Ingrediants);
				...
				//  String Url = strUrl; // "https://www.tarladalal.com/RecipeSearch.aspx?term=high%20blood%20pressure&pageindex="+ p;
				//  driver.get(Url); 
				 // Thread.sleep(6000);
		...
		ExcelReader xlreader = new ExcelReader(".\\datafiles\\Diabetes_addon.xlsx");
		xlreader.setcelldata("Sheet1", 0, 0, "recipeid");
		xlreader.setcelldata("Sheet1", 0, 1, "recipename");
		//xlreader.setCellValue("Sheet2", 0, 2,"Recipe Category(Breakfast/lunch/snack/dinner)");
		...
		xlreader.setcelldata("Sheet1", 0,8,"Targetted morbid conditions Diabeties");
		xlreader.setcelldata("Sheet1", 0, 9, "Url");
				...
				for (int a = 0; a <=recipeName.size(); a++) 
				{
					...
					//xlreader.setcelldata("Sheet1", a+1, 2,"Recipe Category");
